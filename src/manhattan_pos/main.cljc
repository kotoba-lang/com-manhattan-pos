(ns manhattan_pos.main
  "Kotodama WASM entrypoint for the manhattan_pos clean-room actor (L4) — Clojure port.
  ...
  facts (`manhattan_pos.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "manhattan_pos")
(def tier "L4")
...
(def entity-specs
  [{:entity "Product"   :plural "products"   :id-prefix "manhatta_pro"
   ...
   {:entity "Customer"  :plural "customers"  :id-prefix "manhatta_cus"
    :fields [:email :firstName :lastName :phone]        :required [:email :firstName]
    :coerce {}        :refs {}}
   {:entity "Collection" :plural "collections" :id-prefix "manhatta_col"
...
(defn emit-facts
  "EAVT facts for one record: {\"manhattan_pos.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "manhattan_pos-compat" :tier tier :entities entities} 200])
...
(defn start! [] :manhattan_pos-compat/ready)
