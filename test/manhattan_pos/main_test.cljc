(ns manhattan_pos.main-test
  "Contract + behavioral test for the manhattan_pos-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "manhattan_pos-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'manhattan_pos.main-test)]
